...
const UNMOUNT_CLEANUP_DELAY_MS = 50; // Grace period to survive StrictMode's synthetic unmount
...
export function useVapi(book: IBook) {
  ...
  // Tracks whether the hook is currently mounted. Used to distinguish a real
  // unmount from React 18 StrictMode's synthetic mount -> unmount -> remount
  // in dev, which was previously killing live Vapi calls seconds after they
  // started (cleanup fired -> getVapi().stop() -> Daily room deleted).
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Cleanup handlers immediately - safe to re-register on remount
      Object.entries(handlers).forEach(([event, handler]) => {
        getVapi().off(event as keyof typeof handlers, handler as () => void);
      });
      if (timerRef.current) clearInterval(timerRef.current);

      // Defer ending the live call/session. In dev, React 18 StrictMode
      // mounts -> unmounts -> remounts this effect synchronously, which
      // was previously ending the Vapi/Daily call instantly after it
      // started. Waiting a beat lets the remount happen first; if it
      // does, isMountedRef.current will be back to true and we skip
      // tearing down the call. On a genuine unmount, nothing sets it
      // back to true, so the call and session end as intended.
      setTimeout(() => {
        if (isMountedRef.current) return; // remounted for real, not a true unmount
